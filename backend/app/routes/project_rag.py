...
def _parse_quiz_blocks(markdown_text: str) -> list[dict[str, Any]]:
    ...
    for raw_line in (markdown_text or "").splitlines():
        ...
        question_match = re.match(
            r"^(?:-\s*)?(?:question|q|cau\s*hoi|câu\s*hỏi|cau|câu)\s*([1-9]\d*)\s*[:\)\.-]\s*(.+)$",
            line,
            flags=re.IGNORECASE,
        )
        if not question_match:
            question_match = re.match(r"^([1-9]\d*)\s*[:\)\.-]\s*(.+)$", line, flags=re.IGNORECASE)
        if question_match:
            if current and str(current.get("question") or "").strip():
                blocks.append(current)
            current = {
                "question": str(question_match.group(2)).strip(),
                "options": {},
                "answer": "",
                "ideal_answer": "",
                "explanation": "",
        ...
        if line.endswith("?") and (current is None or current.get("options") or current.get("answer")):
            if current and str(current.get("question") or "").strip():
                blocks.append(current)
            current = {
                "question": line,
        ...
        if option_match:
            letter = str(option_match.group(1)).upper()
            option_text = str(option_match.group(2)).strip()
            if option_text:
                current["options"][letter] = option_text
            continue

        answer_match = re.match(r"^(?:Answer|Đáp án|Dap an)\s*[:\-]\s*(.+)$", line, flags=re.IGNORECASE)
        if answer_match:
            current["answer"] = str(answer_match.group(1)).strip()
            continue

        ideal_answer_match = re.match(
            r"^(?:Ideal\s*Answer|Đáp\s*án\s*mẫu|Dap\s*an\s*mau|Gợi\s*ý\s*đáp\s*án|Goi\s*y\s*dap\s*an)\s*[:\-]\s*(.+)$",
            line,
            flags=re.IGNORECASE,
        )
        if ideal_answer_match:
            current["ideal_answer"] = str(ideal_answer_match.group(1)).strip()
            continue

        explanation_match = re.match(
            r"^(?:Explanation|Giải thích|Giai thich)\s*[:\-]\s*(.+)$",
            line,
            flags=re.IGNORECASE,
        )
        if explanation_match:
            current["explanation"] = str(explanation_match.group(1)).strip()
...
def _ensure_question_sentence(text: str) -> str:
    sentence = re.sub(r"\s+", " ", (text or "").strip(" .;,:"))
    if not sentence:
        return "Nội dung chính nào được trình bày trong tài liệu?"
    if not sentence.endswith("?"):
        sentence = f"{sentence}?"
    return sentence


def _build_fallback_mcq_options(question_text: str) -> dict[str, str]:
    # Subject-agnostic fallback options that encourage critical thinking
    return {
        "A": "Lựa chọn chính xác dựa trên các khái niệm được trình bày trong ngữ cảnh.",
        "B": "Một phương án thay thế có vẻ hợp lý nhưng thiếu các điều kiện cần thiết.",
        "C": "Cách tiếp cận ngược lại với lý thuyết cốt lõi của nội dung bài học.",
        "D": "Một khái niệm liên quan nhưng không giải quyết trực tiếp tình huống đặt ra.",
    }


def _infer_mcq_answer_letter(question_text: str, options: dict[str, str]) -> str:
    """Infers the answer letter based on keyword matching between the question focus and options.
    
    This is a fallback mechanism when the LLM response is not perfectly structured.
    """
    focus_queries = _detect_quiz_focus_queries(question_text)
    
    # Map generic concept categories to likely identifying tokens in the options
    expected_tokens_by_query: dict[str, tuple[str, ...]] = {
        "Definition": (" là ", " định nghĩa ", " khái niệm ", " is ", " definition "),
        "Mechanism": (" cách ", " hoạt động ", " vận hành ", " how ", " works "),
        "Comparison": (" khác ", " so sánh ", " phân biệt ", " versus ", " contrast "),
        "Application": (" dùng ", " sử dụng ", " áp dụng ", " use ", " apply "),
    }

    for query in focus_queries:
        # Check both the specific query and its base category
        category = query.split("/")[-1] if "/" in query else query
        expected_tokens = expected_tokens_by_query.get(category, ())
        
        if not expected_tokens:
            continue
            
        for letter in ("A", "B", "C", "D"):
            option = str(options.get(letter) or "")
            normalized_option = f" {_normalize_heading_label(option)} "
            if any(token in normalized_option for token in expected_tokens):
                return letter

    return "A"


def _strip_quiz_reference_markers(text: str) -> str:
    cleaned = str(text or "")
    cleaned = re.sub(
        r"\b(?:muc|mục|section|chuong|chương|phan|phần|vi\s*du|ví\s*dụ|example|trang|page)\s*\d+(?:\.\d+)*\b",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
...
def _enforce_quiz_hard_format(
    ...
    blocks = _parse_quiz_blocks(markdown_text)
    total_questions = 6
    mcq_count = 3
    ...
    question_pool: list[str] = []
    for item in blocks:
        question = _ensure_question_sentence(
            _strip_quiz_reference_markers(str(item.get("question") or "")),
        )
        if question:
            question_pool.append(question)

    for item in _extract_line_candidates(markdown_text):
        cleaned_item = _strip_quiz_reference_markers(item)
        if not cleaned_item:
            continue
        cleaned_item = re.sub(
            r"^(?:question|q|cau\s*hoi|câu\s*hỏi|cau|câu)\s*[1-9]\d*\s*[:\)\.-]\s*",
            "",
            cleaned_item,
            flags=re.IGNORECASE,
        ).strip()
        if not cleaned_item:
            continue
        if re.match(r"^(?:[A-D][\.:\)]|đáp án|dap an|giải thích|giai thich|nguồn|nguon)\b", cleaned_item, flags=re.IGNORECASE):
            continue
        if not cleaned_item.endswith("?"):
            continue
        question_pool.append(_ensure_question_sentence(cleaned_item))
        if len(question_pool) >= max(8, total_questions * 2):
            break

    question_pool = _dedupe_text_items(question_pool)
    if not question_pool:
        question_pool = [
            "Câu lệnh nào dùng để cập nhật dữ liệu trong bảng khi đã có điều kiện lọc?",
            "Trong truy vấn SELECT, mệnh đề nào giúp lọc bản ghi theo điều kiện?",
            "Khi cần thêm dữ liệu mới vào bảng, bạn nên dùng câu lệnh nào?",
            "Mệnh đề ORDER BY có vai trò gì trong truy vấn dữ liệu?",
            "Khi nối dữ liệu từ hai bảng, câu lệnh nào được sử dụng phổ biến?",
            "Trong thực tế, khi nào nên dùng UPDATE kết hợp WHERE để tránh lỗi dữ liệu?",
        ]

    while len(question_pool) < total_questions:
        question_pool.append(question_pool[-1])
    ...
    for idx in range(mcq_count):
        question_no = idx + 1
        block = blocks[idx] if idx < len(blocks) and isinstance(blocks[idx], dict) else {}
        question = question_pool[idx] if idx < len(question_pool) else question_pool[-1]
        question = _ensure_question_sentence(_strip_quiz_reference_markers(question))

        options = block.get("options") if isinstance(block, dict) else {}
        if isinstance(options, dict) and all(letter in options for letter in ("A", "B", "C", "D")):
            resolved_options = {
                letter: _strip_quiz_reference_markers(str(options.get(letter) or ""))
                or _build_fallback_mcq_options(question)[letter]
                for letter in ("A", "B", "C", "D")
            }
        else:
            resolved_options = _build_fallback_mcq_options(question)

        answer_raw = str(block.get("answer") or "").strip().upper() if isinstance(block, dict) else ""
        answer_letter = (
            answer_raw[:1]
            if answer_raw[:1] in {"A", "B", "C", "D"}
            else _infer_mcq_answer_letter(question, resolved_options)
        )

        explanation_raw = str(block.get("explanation") or "") if isinstance(block, dict) else ""
        explanation_text = _ensure_complete_sentence(
            _strip_quiz_reference_markers(_clean_line_candidate(explanation_raw)),
            fallback=(
                f"Đáp án {answer_letter} phù hợp với logic câu lệnh và ngữ cảnh dữ liệu được trình bày trong tài liệu."
            ),
        )
        ...
        lines.append("")
        lines.append(f"A. {resolved_options['A']}")
        lines.append(f"B. {resolved_options['B']}")
        lines.append(f"C. {resolved_options['C']}")
        lines.append(f"D. {resolved_options['D']}")
        lines.append("")
        lines.append(f"Đáp án: {answer_letter}")
        lines.append("")
        lines.append("Giải thích:")
        lines.append(explanation_text)
        lines.append("")
        lines.append(_format_quiz_source_line(selected_record))
    ...
    for idx in range(essay_count):
        question_no = mcq_count + idx + 1
        block_index = question_no - 1
        block = blocks[block_index] if block_index < len(blocks) and isinstance(blocks[block_index], dict) else {}
        question = question_pool[block_index] if block_index < len(question_pool) else question_pool[-1]
        question = _ensure_question_sentence(_strip_quiz_reference_markers(question))

        guidance_raw = str(block.get("ideal_answer") or "") if isinstance(block, dict) else ""
        guidance_text = _ensure_complete_sentence(
            _strip_quiz_reference_markers(_clean_line_candidate(guidance_raw)),
            fallback=(
                "Trình bày hướng xử lý theo cú pháp và logic xuất hiện trong tài liệu, nêu rõ bước thực hiện chính."
            ),
        )
        ...
        explanation_text = _ensure_complete_sentence(
            _strip_quiz_reference_markers(_clean_line_candidate(explanation_raw)),
            fallback=(
                "Giải thích này bám sát thao tác và hành vi câu lệnh trong context, giúp áp dụng đúng vào tình huống thực tế."
            ),
        )
        if not explanation_text:
            explanation_text = (
        ...
        selected_record, source_records = _select_quiz_source_record_for_question(
            question_text=question,
            explanation_text=f"{guidance_text}\n{explanation_text}",
            source_records=source_records,
            used_chunk_ids=used_chunk_ids,
            selected_source_docs=selected_source_docs,
        )
...
def _enforce_main_content_hard_format(
    ...
    # Prepare source records for citation injection
    source_records = _build_quiz_source_records(retrieved_chunks or [])
    ...
    for idx, (raw_title, body_lines, _heading_level) in enumerate(subsections, start=1):
        ...
        for match in re.finditer(r"```([^\n`]*)\n([\s\S]*?)```", body_text, flags=re.MULTILINE):
            ...
            code_text = (match.group(2) or "").strip()
            if code_text:
                code_blocks.append((language, code_text))

        plain_sentences = _extract_plain_sentences(body_text)
        ...
        if not cleaned_sentences and code_blocks:
            cleaned_sentences.append(
                f"{clean_title} được làm rõ qua ví dụ minh họa để người học dễ áp dụng vào bài tập thực tế."
            )
        ...
        for sentence in cleaned_sentences:
            ...
            if len(sentence_buffer) == 2:
                paragraphs.append(" ".join(sentence_buffer).strip())
                sentence_buffer = []
        if sentence_buffer:
            paragraphs.append(" ".join(sentence_buffer).strip())
        ...
        for paragraph_index, paragraph in enumerate(paragraphs):
            ...
            if paragraph_index < len(paragraphs) - 1 or code_blocks:
                output_lines.append("")

        if code_blocks:
            output_lines.append("Ví dụ minh họa:")
            for block_index, (language, code_text) in enumerate(code_blocks):
                output_lines.append(f"```{language}" if language else "```")
                output_lines.append(code_text)
                output_lines.append("```")
                if block_index < len(code_blocks) - 1:
                    output_lines.append("")

        # Inject citation for this subsection
        if source_records:
