    ...
    for model_name in runtime_model_order(pipeline):
        try:
            ...
            if text:
                ...
                try:
                    from ..context import request_usage_var
                    from ..db.stats import estimate_tokens_from_text
                    usage = request_usage_var.get()
                    if usage is not None:
                        usage["llm_calls"] += 1
                        usage["token_usage"] += estimate_tokens_from_text(prompt, text)
                except Exception as usage_err:
                    logger.debug("Failed to record usage in ContextVar: %s", usage_err)
    ...
    for model_name in runtime_model_order(pipeline):
        try:
            ...
            if yielded_anything:
                ...
                # Record LLM call and token usage for streaming
                try:
                    from ..context import request_usage_var
                    from ..db.stats import estimate_tokens_from_text
                    usage = request_usage_var.get()
                    if usage is not None:
                        usage["llm_calls"] += 1
                        usage["token_usage"] += estimate_tokens_from_text(prompt, "".join(full_text))
                except Exception as usage_err:
                    logger.debug("Failed to record usage in ContextVar: %s", usage_err)
