    ...
    >
      ...
      {loading ? (
        ...
        {projects.map((project) => (
          <div
            key={project.id}
            className="bg-white border rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow relative group"
          >
            <div className="flex items-start justify-between mb-4">
            ...
            <div className="flex items-center justify-between mt-4 text-sm text-slate-500">
              ...
              <span className="bg-slate-100 px-2 py-1 rounded-md">
                {project.sections_count || 0} sections
              </span>
            </div>
            <button
              onClick={() => handleOpenEditor(project)}
              className="mt-4 w-full bg-slate-50 hover:bg-blue-50 text-blue-600 font-medium py-2 rounded-lg transition-colors border border-transparent hover:border-blue-200"
            >
              Mở soạn thảo
            </button>
          </div>
                  ...
                  <div className="max-h-36 overflow-y-auto border rounded-lg p-2 space-y-1">
                    {documents.map((doc) => (
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={knowledgeBaseIds.includes(String(doc.id))}
                          onChange={() => toggleKb(String(doc.id))}
                        />
