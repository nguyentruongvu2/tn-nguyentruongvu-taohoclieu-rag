...
export default function AdminDashboard() {
  ...
  const NavItem = ({
    ...
    <button
      ...
      className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 ${activeTab === tabKey ? "bg-purple-600 text-white shadow-md" : "text-gray-400 hover:bg-purple-900 hover:text-purple-300"}`}
    >
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden w-full font-sans">
      ...
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-30 w-72 bg-gray-900 text-white border-r border-gray-800 transform transition-transform duration-300 ease-in-out flex flex-col ${isSidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        ...
        <div className="p-4 border-t border-gray-800 bg-gray-950/30">
          <div className="bg-gray-800/50 rounded-xl p-4 flex flex-col items-center border border-gray-700">
            <div className="w-12 h-12 bg-purple-500/20 rounded-full flex items-center justify-center text-purple-400 font-bold text-lg mb-2">
              {currentUser?.username?.charAt(0).toUpperCase() || "A"}
            </div>
            <p className="text-sm font-bold text-white truncate w-full text-center">
              {currentUser?.username || "Admin"}
            </p>
            <p className="text-xs text-gray-400 mb-4">Super Admin</p>
            <button
              onClick={handleLogout}
              className="flex items-center gap-2 text-sm text-red-400 hover:text-red-300 hover:bg-red-400/10 py-2 px-4 rounded-lg w-full justify-center transition-colors"
            >
